...
#[derive(Parser)]
#[grammar = "glacier_parser/glacier.pest"]
...
fn parse_statement(pair: Pair<Rule>) -> Statement {
    match pair.as_rule() {
        ...
        Rule::empty_return => Statement::EmptyReturn(EmptyReturn {
            pos: pair.as_span(),
        }),
        Rule::nonempty_return => {
            let mut inner = pair.clone().into_inner();
...
fn parse_program(res: Pairs<Rule>) -> Program {
    ...
    for pair in res {
        match pair.as_rule() {
            ...
            | Rule::function_declare
            | Rule::empty_return
            | Rule::nonempty_return => ast.push(parse_statement(pair)),
            _ => {}
        }
    }
    ast
...
#[cfg(test)]
mod tests {
    use crate::glacier_parser::parse;
